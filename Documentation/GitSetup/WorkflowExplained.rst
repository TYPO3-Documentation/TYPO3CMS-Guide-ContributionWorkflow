...
you understand `why` we do things the way the do them. So let us walk you through that.
...
As you can see, your workflow is a bit different, simply because we do not allow to push right back to our GIT repository.
...
#. Reviewing consists of two parts, the **Code Review** where a contributor checks if the code style is according to our
   ...
   is supposed to do (like fixing an issue) and checks whether the unittests run through.
#. If a review has enough positive votes, an **active contributor** (aka member of the coreteam) is able to merge that review
   into the existing codebase. The reason we need a member of the coreteam to finally merge the change is that we run a
   "You break it, you fix it" policy, so the coreteam members that merged a change immediately become responsible for that
